...
/* 代码整理：懒人之家 www.lanrenzhijia.com */
 function EmailAutoComplete(options) {
	...
	this.config = {
		targetCls      :  '.emails',       // 目标input元素
		parentCls      :  '.fuemail',       // 当前input元素的父级类
		hiddenCls      :  '.hiddenCls',       // 当前input隐藏域 
		searchForm     :  '.myform', //form表单
		...
		isSelectHide   : false,                // 点击下拉框 是否隐藏 默认为true
		...
		if(curIndex > -1){
			...
			if(_cache.onlyFlag) {
				...
				$('.auto-tip',cfg.parentNode).css({'position':'absolute','top':cfg.height,'width':cfg.width - 2 + 'px','left':0,
					'border':'1px solid #f69','z-index':10000});
			}
